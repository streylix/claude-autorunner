...
class PlanModeConfigManager {
    ...
    bindEventListeners() {
        ...
        if (exportPythonBtn && exportConfigBtn) {
            exportPythonBtn.addEventListener('click', (e) => {
                this.handlePythonExport();
            });
            ...
            console.log('✅ Export buttons bound');
            boundElements++;
        } else {
            console.warn('⚠️ Export buttons not found');
        }
    ...
    exportConfig() {
        ...
        // Add Python export if PythonExportSystem is available
        if (typeof PythonExportSystem !== 'undefined') {
            try {
                const pythonExporter = new PythonExportSystem();
                const pythonExport = pythonExporter.integrateWithPlanModeConfig(this);
                return pythonExport;
            } catch (error) {
                console.warn('Python export failed:', error);
                return basicExport;
            }
        }
    ...
    exportToPython(options = {}) {
        if (typeof PythonExportSystem === 'undefined') {
            throw new Error('PythonExportSystem not available. Please include python-export-system.js');
        }

        const pythonExporter = new PythonExportSystem();
        const exportOptions = {
            ...pythonExporter.getExportOptionsForPreset(this.currentPreset),
            ...options
        };

        return pythonExporter.export(this.config, exportOptions);
    }

    handlePythonExport() {
        try {
            console.log('🐍 Exporting Python script...');
            const pythonExport = this.exportToPython();
            
            if (pythonExport.success) {
                // Create and trigger download
                const link = document.createElement('a');
                link.href = pythonExport.url;
                link.download = pythonExport.filename;
                ...
                this.showExportMessage('Python script downloaded successfully!', 'success');
                
                // Clean up the blob URL after a delay
                setTimeout(() => {
                    URL.revokeObjectURL(pythonExport.url);
                }, 1000);
            } else {
                throw new Error('Python export failed');
            }
        } catch (error) {
            console.error('❌ Python export failed:', error);
            this.showExportMessage('Failed to export Python script: ' + error.message, 'error');
        }
    }
    ...
    handleConfigExport() {
        try {
            ...
            const configData = JSON.stringify(configExport, null, 2);
            const blob = new Blob([configData], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
            const presetName = this.currentPreset || 'custom';
            const filename = `claude-flow-${presetName}-${timestamp}.json`;
            ...
            this.showExportMessage('Configuration exported successfully!', 'success');
            ...
            this.showExportMessage('Failed to export configuration: ' + error.message, 'error');
        }
    }
